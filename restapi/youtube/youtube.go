...
func getClient(scope string, id string) *http.Client {
	ctx := context.Background()
	b, err := ioutil.ReadFile("tests/test/restapi/youtube/client_secret_"+id+".json")
	if err != nil {
	...
	config.RedirectURL = "http://vtm.polymorphy.ru:8080"
	// Use the following redirect URI if launchWebServer=false in oauth2.go
	// config.RedirectURL = "urn:ietf:wg:oauth:2.0:oob"

	cacheFile, err := tokenCacheFile(id)
	if err != nil {
...
func tokenCacheFile(id string) (string, error) {
	return "tests/test/restapi/youtube/youtube-go_"+id+".json", nil
}
